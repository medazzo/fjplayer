...
function PlayerMedia(fjPlayerId) {
  ...
  function onStreamInitialized() {
    var i = 0;
    var availableTracks = null; // video tracks
...
function PlayerUi(videoContId, VWidth, WHeight) {
  ...
  function onprogressClick(event) {
    var val = 0; // var p = progressBar.value;
  ...
  function initialize(mainPlayer) {
    ...
    progressBar.addEventListener('click', onprogressClick); // window.addEventListener('resize', onResizeWindow);
...
function Thumbs(thtimer, thumbImgElement, thumbDivElement, progressBarElement) {
  ...
  function renderThumbs(event) {
    logger.warn('eventing Thumbs is', event); // first we convert from mouse to time position ..

    var c, i, url, xywh, left;
    var rect = progressBar.getBoundingClientRect();
    var p = (event.pageX - rect.left) * (vidDuration / (rect.right - rect.left));
    var dur = _utils_Utils__WEBPACK_IMPORTED_MODULE_1__.duration(parseFloat(p));
...
"use strict";
module.exports = JSON.parse('{"scripts":{"dev":"webpack  --color --config webpack.config.js ","devw":"webpack  --color --config webpack.config.js  --watch","testPlayer":"NODE_ENV=testing karma run  -- --grep=FjTestPlayer","web":"http-server -a localhost -p 8000 -c-1 ","testPlaylist":"NODE_ENV=testing karma run  -- --grep=FjTestPlaylist","test:watch":"NODE_ENV=testing karma start --single-run=false --auto-watch","testUtils":"NODE_ENV=testing karma run  -- --grep=FjTestUtils","testAll":"NODE_ENV=testing karma run  -- --grep=FjTest","build":"webpack   --color --config webpack.prod.config.js  -p  --optimize-minimize ","testServer":"NODE_ENV=testing karma start ","purify":"webpack  --color --config webpack.purifycss.js --progress  --watch -d","dist":" concurrently \\"npm run build\\" \\"npm run dev\\"","live":"webpack serve --color --config webpack.config.js","stats":"webpack --profile --json > webpack-stats.json"},"version":"0.2.22","private":true,"repository":{"type":"git","url":"https://easysoftin.com"},"description":"the fjplayer html5 dash video player","bugs":{"url":"https://easysoftin.com"},"devDependencies":{"@babel/core":"^7.14.3","@babel/eslint-parser":"^7.14.3","@babel/preset-env":"^7.14.2","babel":"^6.23.0","babel-loader":"^8.2.2","chai":"^4.3.4","css-loader":"^5.2.6","eslint-webpack-plugin":"^2.5.4","file-loader":"^6.2.0","font-awesome":"^4.7.0","karma":"^6.3.2","less":"^4.1.1","less-loader":"^9.0.0","mocka":"^0.0.1","style-loader":"^2.0.0","url-loader":"^4.1.1","webpack":"^5.37.1","webpack-cli":"^4.7.0","webpack-dev-server":"^3.11.2"},"author":"Mohamed Azzouni","name":"fjplayer","homepage":"https://easysoftin.com","license":"MIT","keywords":["mpegdash","es6","dashjs","html5","player","cenc","ads"],"main":"dist/fjplayer.js","dependencies":{"shaka-player":"^3.1.0"}}');
...
fjplayer.Version = _Version__WEBPACK_IMPORTED_MODULE_2__.default;
