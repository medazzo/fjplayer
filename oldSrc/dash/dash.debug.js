            ...
            if (!ks) {
                ...
                if (cp) {
                    ...
                    var pssh = MediaPlayer.dependencies.protection.CommonEncryption.parsePSSHList(initData);
               	  console.warn("Common search in key nbr "+keySystems.length);
      
                    for (ksIdx = 0; ksIdx < keySystems.length; ++ksIdx) {
                    	this.debug.log("In key nbr "+keySystems.length+" , searching in uuid :"+keySystems[ksIdx].uuid);                    	
                        if ( ( keySystems[ksIdx].uuid ==  "10000000-0000-0000-0000-000000000000")  && protectionModel.isSupported(keySystems[ksIdx], mediaInfo.codec)) {
                            ks = keySystems[ksIdx];
...
MediaPlayer.dependencies.protection.KeySystem_ClearKey = function() {
    ...
    var keySystemStr = "webkit-org.w3.clearkey", keySystemUUID = "10000000-0000-0000-0000-000000000000", protData, requestClearKeyLicense = function(message, requestData) {
        ...
        console.warn(">>>> REQUEST ASK CKTYPE "+ckType);
        byteCursor += 1;
        if (ckType === 0) {
            ...
            console.warn(">>>> REQUEST ASK url "+url);
            ...
            xhr.onload = function() {
                if (xhr.status == 200) {
                    if (!xhr.response.hasOwnProperty("keys")) {
                        that.notify(MediaPlayer.dependencies.protection.KeySystem.eventList.ENAME_LICENSE_REQUEST_COMPLETE, null, new Error("DRM: ClearKey Remote update, Illegal response JSON"));
                    }else{
				...
				var Akeyid = Base64.BinToArray(keyid);
				var Akey = Base64.BinToArray(key);
				keyPairs.push(new MediaPlayer.vo.protection.KeyPair(Akeyid, Akey));
				}
	                    var event = new MediaPlayer.vo.protection.LicenseRequestComplete(new MediaPlayer.vo.protection.ClearKeyKeySet(keyPairs), requestData);
                  	    that.notify(MediaPlayer.dependencies.protection.KeySystem.eventList.ENAME_LICENSE_REQUEST_COMPLETE, event);
	                }
                } else {
                    that.notify(MediaPlayer.dependencies.protection.KeySystem.eventList.ENAME_LICENSE_REQUEST_COMPLETE, null, new Error('DRM: ClearKey Remote update, XHR aborted. status is "' + xhr.statusText + '" (' + xhr.status + "), readyState is " + xhr.readyState));
                }
            };
    ...
    return {
        ...
        initDataEquals: function(initData1, initData2) {
            if (initData1.length === initData2.length) {
                if (Base64.encode(initData1.buffer) === Base64.encode(initData2.buffer)) {
                    return true;
                }
            }
            return false;
...
MediaPlayer.vo.protection.ClearKeyKeySet = function(keyPairs, type) {
    ...
    this.toJWKString = function() {
	    ...
	    console.warn(" >> keyPairs[i].keyID >>"+keyPairs[i].keyID);
	    console.warn(" >> keyPairs[i].key >>"+keyPairs[i].key);
	    ...
	    onsole.warn(" >> key.k  >>"+key.k );
	    console.warn(" >> key.kid  >>"+key.kid );
...
MediaPlayer.vo.protection.KeyPair = function(keyID, key) {
	console.warn("KID  length is "+keyID.length );
	console.warn("KEY  length is "+key.length );
    "use strict";
